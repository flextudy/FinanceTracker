  ...
  return (
    <div className="relative min-w-0">
      <select
        ...
        className="w-full appearance-none rounded-[14px] border border-[#c0bbb6] bg-white py-2.5 pl-3 pr-8 text-sm font-medium text-[#615f5c] outline-none focus:border-[#fa5d00] focus:ring-2 focus:ring-[#fa5d00]/15"
      >
        {children}
      </select>
            ...
            <div className="mb-4 rounded-[20px] border border-[#e3d6c5] bg-white p-3 shadow-sm sm:p-4">
              <div className="flex flex-col gap-3 lg:flex-row">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex sm:gap-3">
                  <Select label="Filter by date" value={date} onChange={setDate}>
                    <option>All dates</option>
                    <option>August 2026</option>
                  </Select>
                  <Select label="Filter by partner" value={partner} onChange={setPartner}>
            ...
            {isLoading ? (
              ...
              <>
                ...
                <div className="space-y-3 md:hidden">
                  {filtered.map((settlement) => (
                    <SettlementMobileCard key={settlement.id} settlement={settlement} />
                  ))}
                </div>
              </>
            ) : (
...
function SettlementMobileCard({ settlement }: { settlement: Settlement }) {
  return (
    <Card className="p-4 border border-[#e3d6c5] shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <p className="text-lg font-bold text-[#1d1e1c]">{money(settlement.amount)}</p>
      </div>
      <div className="mt-3">
